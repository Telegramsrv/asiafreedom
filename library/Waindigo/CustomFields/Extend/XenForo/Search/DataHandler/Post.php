...
class Waindigo_CustomFields_Extend_XenForo_Search_DataHandler_Post extends XFCP_Waindigo_CustomFields_Extend_XenForo_Search_DataHandler_Post
{
    ...
    public function getTypeConstraintsFromInput(XenForo_Input $input)
    {
        ...
        if ($threadFields) {
            foreach ($threadFields as $fieldId => $fieldValue) {
                if ($fieldValue) {
                    $constraints['thread_field_id_' . $fieldId] = $fieldId;
                    $constraints['thread_field_value_' . $fieldId] = $fieldValue;
                }
            }
        }
    ...
    public function processConstraint(XenForo_Search_SourceHandler_Abstract $sourceHandler, $constraint, $constraintInfo,
        array $constraints)
    {
        if (strlen($constraint) > strlen('thread_field_id_') &&
             substr($constraint, 0, strlen('thread_field_id_')) == 'thread_field_id_') {
            if ($constraintInfo) {
                $constraintInfo = strval($constraintInfo);
                return array(
                    'query' => array(
                        'thread_field_value_' . $constraintInfo,
                        'field_id',
                        '=',
                        $constraintInfo
                    )
        ...
        if (strlen($constraint) > strlen('thread_field_value_') &&
             substr($constraint, 0, strlen('thread_field_value_')) == 'thread_field_value_') {
            if ($constraintInfo) {
