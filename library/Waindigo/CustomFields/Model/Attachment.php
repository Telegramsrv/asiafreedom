...
class Waindigo_CustomFields_Model_Attachment extends XenForo_Model
{
    ...
    public function getCachedFieldAttachmentIdsForContentType($contentType)
    {
        ...
        return array();
    } /* END getCachedFieldAttachmentIdsForContentType */

    public function getFileParams(array $version, array $contentData, array $viewingUser = null)
    {
        return array(
            'hash' => md5(uniqid('', true)), /* END 'hash' */
            'content_type' => 'custom_field', /* END 'content_type' */
            'content_data' => $contentData, /* END 'content_data' */
        );
    } /* END getFileParams */
    ...
    } /* END getFileConstraints */

    public function associateAttachments($contentId, $customFieldType)
    {
        $fieldAttachmentIds = $this->getModelFromCache('Waindigo_CustomFields_Model_Attachment')->getCachedFieldAttachmentIdsForContentType(
            $customFieldType);
