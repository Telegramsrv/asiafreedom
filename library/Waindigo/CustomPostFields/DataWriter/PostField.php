...
/**
* Data writer for custom post fields.
*/
class Waindigo_CustomPostFields_DataWriter_PostField extends Waindigo_CustomFields_DataWriter_AbstractField
{
     ...
     * field.
     *
     * @return Waindigo_CustomPostFields_Definition_PostField
     */
    public function getFieldDefinition()
    {
        return new Waindigo_CustomPostFields_Definition_PostField();
    ...
    protected function _getFields()
    {
        ...
        $fields['xf_post_field']['below_title_on_thread_create'] = array(
            'type' => self::TYPE_UINT,
            'default' => 0
        );
        return $fields;
    ...
    protected function _getExistingData($data)
    {
        ...
        return array(
            'xf_post_field' => $this->_getFieldModel()->getPostFieldById($id)
        );
    ...
    protected function _verifyFieldId(&$id)
    {
        ...
        if ($id !== $this->getExisting('field_id') && $this->_getFieldModel()->getPostFieldById($id)) {
    ...
    protected function _preSave()
    {
        ...
        if ($this->isUpdate() && $this->isChanged('field_type')) {
            $typeMap = $this->_getFieldModel()->getPostFieldTypeMap();
        ...
        if (!$this->get('field_choices_callback_class') && !$this->get('field_choices_callback_method') &&
             in_array($this->get('field_type'), array(
                'select',
                'radio',
                'checkbox',
                'multiselect'
    ...
    protected function _postDelete()
    {
        ...
        $this->_db->delete('xf_post_field_value', 'field_id = ' . $this->_db->quote($fieldId));
        // note the post caches aren't rebuilt here; this shouldn't be an issue
    // as we don't enumerate them
    } /* END _postDelete */
    ...
    {
        return $this->_getFieldModel()->getPostFieldTitlePhraseName($id);
    ...
    {
        return $this->_getFieldModel()->getPostFieldDescriptionPhraseName($id);
    ...
    {
        return $this->_getFieldModel()->getPostFieldChoicePhraseName($fieldId, $choice);
    } /* END _getChoicePhraseName */

    protected function _rebuildPostFieldCache()
    {
        return $this->_getFieldModel()->rebuildPostFieldCache();
    } /* END _rebuildPostFieldCache */
