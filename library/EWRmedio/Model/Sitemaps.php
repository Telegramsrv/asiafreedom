...
class EWRmedio_Model_Sitemaps extends XenForo_Model
{
	public function getRSSbyMedia($media = null, $type = null, $where = null, $local = false)
	{
		...
		{
			$title .= htmlspecialchars(' - '.new XenForo_Phrase('service').': '.$service['service_name']);
		...
		$document->appendChild($rss_node = $document->createElement('rss'));
			...
			$rss_node->appendChild($chn_node = $document->createElement('channel'));
				...
				else
				{
					...
					$medias = $this->getModelFromCache('EWRmedio_Model_Lists')->getMediaList(1, 20, $listParams);
	...
	public function buildRSSbyMedia($document, $media, $local = false)
	{
		$itm_node = $document->createElement('item');
			...
			$sub_node = $document->createElement('description', '');
			$sub_node->appendChild($document->createCDATASection(substr(XenForo_Helper_String::bbCodeStrip(str_replace("\n", " ", $media['media_description'])), 0, 2048)));
			$itm_node->appendChild($sub_node);
			
			$itm_node->appendChild($document->createElement('link', XenForo_Link::buildPublicLink('full:media/media', $media)));
			$itm_node->appendChild($document->createElement('guid', XenForo_Link::buildPublicLink('full:media/media', $media)));
			$itm_node->appendChild($document->createElement('comments', XenForo_Link::buildPublicLink('full:media/media/comments', $media)));
